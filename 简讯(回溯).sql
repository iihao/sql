...
with [bu] AS (
	select BU.BUGUID,
		   BU.BUName,
		   BU.ParentGUID,
		   parent.BuName as ParentName,
		   bu.HierarchyCode,
		   bu.Level,
		   BU.IsEndCompany
	from data_wide_mdm_BusinessUnit BU WITH (NOLOCK)
			 left join data_wide_mdm_BusinessUnit parent WITH (NOLOCK) on BU.ParentGUID = parent.BUGUID
	WHERE BU.BUType = 1
),
	 -- 累计实际回款
	 [ActualGetIn] as (
		 SELECT
			 /* 昨日回款 */
			 isnull(SUM(ISNULL(case when Cast(StatisticalDate as Date) = @yesterday and TopProductTypeName='车库/库房'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_车位昨日],
			 isnull(SUM(ISNULL(case when Cast(StatisticalDate as Date) = @yesterday and TopProductTypeName='住宅'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_住宅昨日],
			 isnull(SUM(ISNULL(case when Cast(StatisticalDate as Date) = @yesterday and TopProductTypeName='商业'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_商业昨日],
			 ...
			 /* 本日回款 */
			 isnull(SUM(ISNULL(case when Cast(StatisticalDate as Date) = @now and TopProductTypeName='车库/库房'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_车位本日],
			 isnull(SUM(ISNULL(case when Cast(StatisticalDate as Date) = @now and TopProductTypeName='住宅'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_住宅本日],
			 isnull(SUM(ISNULL(case when Cast(StatisticalDate as Date) = @now and TopProductTypeName='商业'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_商业本日],
			 ...
			 /* 本周回款 */
			 isnull(SUM(ISNULL(case when (StatisticalDate between @weekstartday and @weekendday) and TopProductTypeName='车库/库房'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_车位本周],
			 isnull(SUM(ISNULL(case when (StatisticalDate between @weekstartday and @weekendday) and TopProductTypeName='住宅'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_住宅本周],
			 isnull(SUM(ISNULL(case when (StatisticalDate between @weekstartday and @weekendday) and TopProductTypeName='商业'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_商业本周],
			 ...
			 /* 本月回款 */
			 isnull(SUM(ISNULL(case when (StatisticalDate between @monthstartday and @monthendday) and TopProductTypeName='车库/库房'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_车位本月],
			 isnull(SUM(ISNULL(case when (StatisticalDate between @monthstartday and @monthendday) and TopProductTypeName='住宅'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_住宅本月],
			 isnull(SUM(ISNULL(case when (StatisticalDate between @monthstartday and @monthendday) and TopProductTypeName='商业'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_商业本月],
			 ...
			 /* 本季回款 */
			 isnull(SUM(ISNULL(case when (StatisticalDate between @quarterstartday and @quarterendday) and TopProductTypeName='车库/库房'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_车位本季],
			 isnull(SUM(ISNULL(case when (StatisticalDate between @quarterstartday and @quarterendday) and TopProductTypeName='住宅'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_住宅本季],
			 isnull(SUM(ISNULL(case when (StatisticalDate between @quarterstartday and @quarterendday) and TopProductTypeName='商业'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_商业本季],
			 ...
			 /* 本年回款 */
			 isnull(SUM(ISNULL(case when (StatisticalDate between @yearstartday and @yearendday) and TopProductTypeName='车库/库房'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_车位本年],
			 isnull(SUM(ISNULL(case when (StatisticalDate between @yearstartday and @yearendday) and TopProductTypeName='住宅'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_住宅本年],
			 isnull(SUM(ISNULL(case when (StatisticalDate between @yearstartday and @yearendday) and TopProductTypeName='商业'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_商业本年],
			 ...
			 /* 累计回款 */
			 isnull(SUM(ISNULL(case when TopProductTypeName='车库/库房'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_车位累计],
			 isnull(SUM(ISNULL(case when TopProductTypeName='住宅'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_住宅累计],
			 isnull(SUM(ISNULL(case when TopProductTypeName='商业'  then  HousingAmount else 0 end, 0)), 0)   AS [贷款类回款金额_商业累计],
	 ...
	 [ActualOrder] as (
		 ...
		 from (
				  SELECT case when isAttachRoom = 0 then OCjTotal else 0 end AS ActualOAmount,
						 ...
						 case when isAttachRoom = 1 then Ts else 0 end as ActualAttachRoomOTs,
						 HsTjDate,
						 BUGUID,
						 BUName
				  FROM dbo.data_wide_s_SaleHsData WITH (NOLOCK)
	 ...
	 [detailResult] as (
		 ...
		 SELECT
			 ...
			 bu.ParentGUID            as [上级GUID],
			 bu.BUGUID                as [公司GUID],
			 bu.BUName                as [公司名称],
			 bu.HierarchyCode         as [排序号],
			 bu.Level                 as [级别],
			 bu.ParentName            as [上级名称]
		 FROM bu
				  left join ActualGetIn on bu.BUGUID = ActualGetIn.BUGUID
				  left join ActualOrder on bu.BUGUID = ActualOrder.BUGUID
				  left join ActualContract on bu.BUGUID = ActualContract.BUGUID
		 where bu.IsEndCompany = 1
	 ),
	 ...
	 [Result]  as (
		 ...
		 from detailResult
		 union all
		 select isnull(贷款类回款金额_昨日, 0)    as [贷款类回款金额_昨日],
				isnull(非贷款类回款金额_昨日, 0)   as [非贷款类回款金额_昨日],
				isnull(补充协议款_昨日, 0)      as 补充协议款_昨日,
				isnull(回款金额_昨日, 0)       as 回款金额_昨日,
				isnull([楼款_模糊匹配_昨日],0)  as [楼款_模糊匹配_昨日],
				...
				bu.buname                as [公司名称],
				bu.BUGUID                AS [公司编号],
				bu.ParentGUID            AS [上级编号],
				bu.ParentName            as [上级名称],
				bu.Level                 as [级别]
		 from Result
				  join bu on bu.BUGUID = Result.[上级编号]
		 where bu.IsEndCompany = 0
		   and bu.Level >= 0
	 )
...
/* 累计签约 */
	...
	[级别]
from (
		 select isnull(sum(isnull(贷款类回款金额_昨日, 0)), 0)    as [贷款类回款金额_昨日],
				...
				isnull(sum(isnull([认购金额_昨日], 0)), 0)     as 认购金额_昨日,
				isnull(sum(isnull([主房间认购金额_昨日], 0)), 0)  as 主房间认购金额_昨日,
				isnull(sum(isnull([附属房间认购金额_昨日], 0)), 0) as 附属房间认购金额_昨日,
				isnull(sum(isnull([主房间认购面积_昨日], 0)), 0)  as 主房间认购面积_昨日,
				isnull(sum(isnull([附属房间认购面积_昨日], 0)), 0) as 附属房间认购面积_昨日,
				isnull(sum(isnull([主房间认购套数_昨日], 0)), 0)  as 主房间认购套数_昨日,
				isnull(sum(isnull([附属房间认购套数_昨日], 0)), 0) as 附属房间认购套数_昨日,
				isnull(sum(isnull([认购金额_本日], 0)), 0)     as 认购金额_本日,
				isnull(sum(isnull([主房间认购金额_本日], 0)), 0)  as 主房间认购金额_本日,
				isnull(sum(isnull([附属房间认购金额_本日], 0)), 0) as 附属房间认购金额_本日,
				isnull(sum(isnull([主房间认购面积_本日], 0)), 0)  as 主房间认购面积_本日,
				isnull(sum(isnull([附属房间认购面积_本日], 0)), 0) as 附属房间认购面积_本日,
				isnull(sum(isnull([主房间认购套数_本日], 0)), 0)  as 主房间认购套数_本日,
				isnull(sum(isnull([附属房间认购套数_本日], 0)), 0) as 附属房间认购套数_本日,
				isnull(sum(isnull([认购金额_本周], 0)), 0)     as 认购金额_本周,
				isnull(sum(isnull([主房间认购金额_本周], 0)), 0)  as 主房间认购金额_本周,
				isnull(sum(isnull([附属房间认购金额_本周], 0)), 0) as 附属房间认购金额_本周,
				isnull(sum(isnull([主房间认购面积_本周], 0)), 0)  as 主房间认购面积_本周,
				isnull(sum(isnull([附属房间认购面积_本周], 0)), 0) as 附属房间认购面积_本周,
				isnull(sum(isnull([主房间认购套数_本周], 0)), 0)  as 主房间认购套数_本周,
				isnull(sum(isnull([附属房间认购套数_本周], 0)), 0) as 附属房间认购套数_本周,
				isnull(sum(isnull([认购金额_本月], 0)), 0)     as 认购金额_本月,
				isnull(sum(isnull([主房间认购金额_本月], 0)), 0)  as 主房间认购金额_本月,
				isnull(sum(isnull([附属房间认购金额_本月], 0)), 0) as 附属房间认购金额_本月,
				isnull(sum(isnull([主房间认购面积_本月], 0)), 0)  as 主房间认购面积_本月,
				isnull(sum(isnull([附属房间认购面积_本月], 0)), 0) as 附属房间认购面积_本月,
				isnull(sum(isnull([主房间认购套数_本月], 0)), 0)  as 主房间认购套数_本月,
				isnull(sum(isnull([附属房间认购套数_本月], 0)), 0) as 附属房间认购套数_本月,
				isnull(sum(isnull([认购金额_本季], 0)), 0)     as 认购金额_本季,
				isnull(sum(isnull([主房间认购金额_本季], 0)), 0)  as 主房间认购金额_本季,
				isnull(sum(isnull([附属房间认购金额_本季], 0)), 0) as 附属房间认购金额_本季,
				isnull(sum(isnull([主房间认购面积_本季], 0)), 0)  as 主房间认购面积_本季,
				isnull(sum(isnull([附属房间认购面积_本季], 0)), 0) as 附属房间认购面积_本季,
				isnull(sum(isnull([主房间认购套数_本季], 0)), 0)  as 主房间认购套数_本季,
				isnull(sum(isnull([附属房间认购套数_本季], 0)), 0) as 附属房间认购套数_本季,
				isnull(sum(isnull([认购金额_本年], 0)), 0)     as 认购金额_本年,
				isnull(sum(isnull([主房间认购金额_本年], 0)), 0)  as 主房间认购金额_本年,
				isnull(sum(isnull([附属房间认购金额_本年], 0)), 0) as 附属房间认购金额_本年,
				isnull(sum(isnull([主房间认购面积_本年], 0)), 0)  as 主房间认购面积_本年,
				isnull(sum(isnull([附属房间认购面积_本年], 0)), 0) as 附属房间认购面积_本年,
				isnull(sum(isnull([主房间认购套数_本年], 0)), 0)  as 主房间认购套数_本年,
				isnull(sum(isnull([附属房间认购套数_本年], 0)), 0) as 附属房间认购套数_本年,
				isnull(sum(isnull([认购金额_累计], 0)), 0)     as 认购金额_累计,
				isnull(sum(isnull([主房间认购金额_累计], 0)), 0)  as 主房间认购金额_累计,
				isnull(sum(isnull([附属房间认购金额_累计], 0)), 0) as 附属房间认购金额_累计,
				isnull(sum(isnull([主房间认购面积_累计], 0)), 0)  as 主房间认购面积_累计,
				isnull(sum(isnull([附属房间认购面积_累计], 0)), 0) as 附属房间认购面积_累计,
				isnull(sum(isnull([主房间认购套数_累计], 0)), 0)  as 主房间认购套数_累计,
				isnull(sum(isnull([附属房间认购套数_累计], 0)), 0) as 附属房间认购套数_累计,

				isnull(sum(isnull([签约金额_昨日], 0)), 0)     as 签约金额_昨日,
				isnull(sum(isnull([主房间签约金额_昨日], 0)), 0)  as 主房间签约金额_昨日,
				isnull(sum(isnull([附属房间签约金额_昨日], 0)), 0) as 附属房间签约金额_昨日,
				isnull(sum(isnull([主房间签约面积_昨日], 0)), 0)  as 主房间签约面积_昨日,
				isnull(sum(isnull([附属房间签约面积_昨日], 0)), 0) as 附属房间签约面积_昨日,
				isnull(sum(isnull([主房间签约套数_昨日], 0)), 0)  as 主房间签约套数_昨日,
				isnull(sum(isnull([附属房间签约套数_昨日], 0)), 0) as 附属房间签约套数_昨日,
				isnull(sum(isnull([签约金额_本日], 0)), 0)     as 签约金额_本日,
				isnull(sum(isnull([主房间签约金额_本日], 0)), 0)  as 主房间签约金额_本日,
				isnull(sum(isnull([附属房间签约金额_本日], 0)), 0) as 附属房间签约金额_本日,
				isnull(sum(isnull([主房间签约面积_本日], 0)), 0)  as 主房间签约面积_本日,
				isnull(sum(isnull([附属房间签约面积_本日], 0)), 0) as 附属房间签约面积_本日,
				isnull(sum(isnull([主房间签约套数_本日], 0)), 0)  as 主房间签约套数_本日,
				isnull(sum(isnull([附属房间签约套数_本日], 0)), 0) as 附属房间签约套数_本日,
				isnull(sum(isnull([签约金额_本周], 0)), 0)     as 签约金额_本周,
				isnull(sum(isnull([主房间签约金额_本周], 0)), 0)  as 主房间签约金额_本周,
				isnull(sum(isnull([附属房间签约金额_本周], 0)), 0) as 附属房间签约金额_本周,
				isnull(sum(isnull([主房间签约面积_本周], 0)), 0)  as 主房间签约面积_本周,
				isnull(sum(isnull([附属房间签约面积_本周], 0)), 0) as 附属房间签约面积_本周,
				isnull(sum(isnull([主房间签约套数_本周], 0)), 0)  as 主房间签约套数_本周,
				isnull(sum(isnull([附属房间签约套数_本周], 0)), 0) as 附属房间签约套数_本周,
				isnull(sum(isnull([签约金额_本月], 0)), 0)     as 签约金额_本月,
				isnull(sum(isnull([主房间签约金额_本月], 0)), 0)  as 主房间签约金额_本月,
				isnull(sum(isnull([附属房间签约金额_本月], 0)), 0) as 附属房间签约金额_本月,
				isnull(sum(isnull([主房间签约面积_本月], 0)), 0)  as 主房间签约面积_本月,
				isnull(sum(isnull([附属房间签约面积_本月], 0)), 0) as 附属房间签约面积_本月,
				isnull(sum(isnull([主房间签约套数_本月], 0)), 0)  as 主房间签约套数_本月,
				isnull(sum(isnull([附属房间签约套数_本月], 0)), 0) as 附属房间签约套数_本月,
				isnull(sum(isnull([签约金额_本季], 0)), 0)     as 签约金额_本季,
				isnull(sum(isnull([主房间签约金额_本季], 0)), 0)  as 主房间签约金额_本季,
				isnull(sum(isnull([附属房间签约金额_本季], 0)), 0) as 附属房间签约金额_本季,
				isnull(sum(isnull([主房间签约面积_本季], 0)), 0)  as 主房间签约面积_本季,
				isnull(sum(isnull([附属房间签约面积_本季], 0)), 0) as 附属房间签约面积_本季,
				isnull(sum(isnull([主房间签约套数_本季], 0)), 0)  as 主房间签约套数_本季,
				isnull(sum(isnull([附属房间签约套数_本季], 0)), 0) as 附属房间签约套数_本季,
				isnull(sum(isnull([签约金额_本年], 0)), 0)     as 签约金额_本年,
				isnull(sum(isnull([主房间签约金额_本年], 0)), 0)  as 主房间签约金额_本年,
				isnull(sum(isnull([附属房间签约金额_本年], 0)), 0) as 附属房间签约金额_本年,
				isnull(sum(isnull([主房间签约面积_本年], 0)), 0)  as 主房间签约面积_本年,
				isnull(sum(isnull([附属房间签约面积_本年], 0)), 0) as 附属房间签约面积_本年,
				isnull(sum(isnull([主房间签约套数_本年], 0)), 0)  as 主房间签约套数_本年,
				isnull(sum(isnull([附属房间签约套数_本年], 0)), 0) as 附属房间签约套数_本年,
				isnull(sum(isnull([签约金额_累计], 0)), 0)     as 签约金额_累计,
				isnull(sum(isnull([主房间签约金额_累计], 0)), 0)  as 主房间签约金额_累计,
				isnull(sum(isnull([附属房间签约金额_累计], 0)), 0) as 附属房间签约金额_累计,
				isnull(sum(isnull([主房间签约面积_累计], 0)), 0)  as 主房间签约面积_累计,
				isnull(sum(isnull([附属房间签约面积_累计], 0)), 0) as 附属房间签约面积_累计,
				isnull(sum(isnull([主房间签约套数_累计], 0)), 0)  as 主房间签约套数_累计,
				isnull(sum(isnull([附属房间签约套数_累计], 0)), 0) as 附属房间签约套数_累计,
				[排序号],
				[公司名称],
				[公司编号],
				[上级编号],
