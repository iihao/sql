...
RG AS (
	SELECT 
		...
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  AND T.TopProductTypeName='住宅'  THEN ts ELSE 0 END) AS 本周住宅认购套数 ,
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  AND T.TopProductTypeName='住宅'  THEN BldArea ELSE 0 END) AS 本周住宅认购面积 ,
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  AND T.TopProductTypeName='住宅' THEN OCjTotal ELSE 0 END)/10000 AS 本周住宅认购金额 ,	
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday   AND T.TopProductTypeName='住宅'  THEN ts ELSE 0 END) AS 本月住宅认购套数 ,
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday   AND T.TopProductTypeName='住宅'  THEN BldArea ELSE 0 END) AS 本月住宅认购面积 ,
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday   AND T.TopProductTypeName='住宅' THEN OCjTotal ELSE 0 END)/10000 AS 本月住宅认购金额 ,	
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  AND T.TopProductTypeName='住宅'  THEN ts ELSE 0 END) AS 本年住宅认购套数 ,
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  AND T.TopProductTypeName='住宅'  THEN BldArea ELSE 0 END) AS 本年住宅认购面积 ,
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  AND T.TopProductTypeName='住宅' THEN OCjTotal ELSE 0 END)/10000 AS 本年住宅认购金额 ,	
		...
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  AND T.TopProductTypeName='商业'  THEN ts ELSE 0 END) AS 本周商业认购套数 ,
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  AND T.TopProductTypeName='商业'  THEN BldArea ELSE 0 END) AS 本周商业认购面积 ,
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  AND T.TopProductTypeName='商业' THEN OCjTotal ELSE 0 END)/10000 AS 本周商业认购金额 ,	
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday   AND T.TopProductTypeName='商业'  THEN ts ELSE 0 END) AS 本月商业认购套数 ,
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday   AND T.TopProductTypeName='商业'  THEN BldArea ELSE 0 END) AS 本月商业认购面积 ,
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday   AND T.TopProductTypeName='商业' THEN OCjTotal ELSE 0 END)/10000 AS 本月商业认购金额 ,	
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  AND T.TopProductTypeName='商业'  THEN ts ELSE 0 END) AS 本年商业认购套数 ,
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  AND T.TopProductTypeName='商业'  THEN BldArea ELSE 0 END) AS 本年商业认购面积 ,
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  AND T.TopProductTypeName='商业' THEN OCjTotal ELSE 0 END)/10000 AS 本年商业认购金额 ,	
		...
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  AND T.TopProductTypeName='车库/库房'  THEN ts ELSE 0 END) AS 本周车位认购套数 ,
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  AND T.TopProductTypeName='车库/库房'   THEN BldArea ELSE 0 END) AS 本周车位认购面积 ,
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  AND T.TopProductTypeName='车库/库房'  THEN OCjTotal ELSE 0 END)/10000 AS 本周车位认购金额 ,	
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday   AND T.TopProductTypeName='车库/库房'   THEN ts ELSE 0 END) AS 本月车位认购套数 ,
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday   AND T.TopProductTypeName='车库/库房'   THEN BldArea ELSE 0 END) AS 本月车位认购面积 ,
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday   AND T.TopProductTypeName='车库/库房'  THEN OCjTotal ELSE 0 END)/10000 AS 本月车位认购金额 ,	
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  AND T.TopProductTypeName='车库/库房'   THEN ts ELSE 0 END) AS 本年车位认购套数 ,
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  AND T.TopProductTypeName='车库/库房'   THEN BldArea ELSE 0 END) AS 本年车位认购面积 ,
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  AND T.TopProductTypeName='车库/库房'  THEN OCjTotal ELSE 0 END)/10000 AS 本年车位认购金额 ,	
...
新旅定制销售回溯表	已完成开发并完成数据核对
鑫城报表	已完成开发并完成数据核对
永康报表，修改6张	已完成
中大财务接口	接口传输问题产品已推包并验证完成
赣电财务接口	上线问题处理
赣电定制报表专项（7张）	完成销售日报周报，待数据核对


"1、九颂驻场，日常问题支持处理，周例会，开票系统方案汇报时间敲定
2、巡检语句固化到运维平台"
1、硕丰巡检结果前期沟通
1、华熙的更新包问题跟进以及数据修复方法跟进
1、其他客户问题处理跟进
1、兴旺，置境，花园服务交接
1、置境，花园，南崇
...
"1、完成新旅大屏—货值盘点模块、供应货值和去化模块
2、修改完善转正ppt
"
"1、浩城的日报表修改为回溯口径
2、修改新旅大屏—货值盘点模块、供应货值和去化模块
3、转正考核内部演练
"
"1、制作新旅大屏—储备货值分析、在途货值分析等模块。项目货值明细穿透屏模块，完成50%
2、修复浩城的日报表认购金额不对的问题
"
1、制作新旅大屏—储备货值分析、在途货值分析等模块。完成100%
1、新旅大屏校验数据  2、
中大财务接口扣款分录二开完成
华熙报表使用协助
上饶、九江客户服务器加固完成
