...
QY AS (
	SELECT 
		...
		/*以下为汉港定制*/
		SUM(CASE WHEN HsTjDate = @now  AND T.TopProductTypeName='住宅' AND isnull(T.CDiscountRemark,'') like '%老带新%' THEN ts ELSE 0 END) AS 本日老带新签约套数 ,
		SUM(CASE WHEN HsTjDate = @now  AND T.TopProductTypeName='住宅' AND isnull(T.qudao,'')<>'' THEN ts ELSE 0 END) AS 本日渠道签约套数 ,
		SUM(CASE WHEN HsTjDate = @now  AND T.TopProductTypeName='住宅' AND isnull(T.qudao,'')='' AND isnull(T.CDiscountRemark,'') not like '%老带新%' THEN 1 ELSE 0 END) AS 本日其他途径签约套数 ,

		SUM(CASE WHEN   T.TopProductTypeName='住宅' AND isnull(T.CDiscountRemark,'') like '%老带新%' THEN ts ELSE 0 END) AS 累计老带新签约套数 ,
		SUM(CASE WHEN   T.TopProductTypeName='住宅' AND isnull(T.qudao,'')<>'' THEN ts ELSE 0 END) AS 累计渠道签约套数 ,
		SUM(CASE WHEN   T.TopProductTypeName='住宅' AND isnull(T.qudao,'')='' AND isnull(T.CDiscountRemark,'') not like '%老带新%' THEN ts ELSE 0 END) AS 累计其他途径签约套数 ,
		...
		SUM(CASE WHEN HsTjDate = @now AND T.TopProductTypeName='车位'  THEN ts ELSE 0 END) AS 本日车位签约套数 ,
		SUM(CASE WHEN HsTjDate = @now AND T.TopProductTypeName='车位'  THEN ts ELSE 0 END) AS 本日车位签约面积 ,
		SUM(CASE WHEN HsTjDate = @now AND T.TopProductTypeName='车位' THEN RmbAmount ELSE 0 END)/10000 AS 本日车位签约金额 ,
		SUM(CASE WHEN T.TopProductTypeName='车位'  THEN ts ELSE 0 END) AS 累计车位签约套数 ,
		SUM(CASE WHEN T.TopProductTypeName='车位'  THEN BldArea ELSE 0 END) AS 累计车位签约面积 ,
		SUM(CASE WHEN T.TopProductTypeName='车位'  THEN RmbAmount ELSE 0 END)/10000 AS 累计车位签约金额 ,
		---------
	...
	LEFT JOIN (SELECT TradeGUID,max(TopProductTypeName) as TopProductTypeName ,
		max(qudao) as qudao, max(CDiscountRemark) as CDiscountRemark FROM data_wide_s_Trade group by TradeGUID) T 
...
RG AS (
	SELECT 
		...
		SUM(CASE WHEN HsTjDate = @now  THEN RoomTotal ELSE 0 END)/10000  AS 本日认购金额 ,
		...
		SUM(CASE WHEN HsTjDate = @now and T.TopProductTypeName='住宅'  THEN RoomTotal ELSE 0 END)/10000  AS 本日住宅认购金额 ,
		...
		SUM(CASE WHEN HsTjDate = @now and T.TopProductTypeName='商业'  THEN RoomTotal ELSE 0 END)/10000  AS 本日商业认购金额 ,
		SUM(CASE WHEN HsTjDate = @now and T.TopProductTypeName='车位'  THEN ts ELSE 0 END) AS 本日车位认购套数 ,
		SUM(CASE WHEN HsTjDate = @now and T.TopProductTypeName='车位'  THEN BldArea ELSE 0 END) AS 本日车位认购面积 ,
		SUM(CASE WHEN HsTjDate = @now and T.TopProductTypeName='车位'  THEN RoomTotal ELSE 0 END)/10000  AS 本日车位认购金额 ,
		...
		SUM(CASE WHEN   T.TopProductTypeName='住宅'  THEN RoomTotal ELSE 0 END)/10000  AS 累计住宅认购金额 ,
		...
		SUM(CASE WHEN   T.TopProductTypeName='商业'  THEN RoomTotal ELSE 0 END)/10000  AS 累计商业认购金额 ,
		SUM(CASE WHEN   T.TopProductTypeName='车位'  THEN ts ELSE 0 END) AS 累计车位认购套数 ,
		SUM(CASE WHEN   T.TopProductTypeName='车位'  THEN BldArea ELSE 0 END) AS 累计车位认购面积 ,
		SUM(CASE WHEN   T.TopProductTypeName='车位'  THEN RoomTotal ELSE 0 END)/10000  AS 累计车位认购金额 ,
		...
		SUM(CASE WHEN HsTjDate BETWEEN @weekstartday AND @weekendday  THEN RoomTotal ELSE 0 END)/10000  AS 本周认购金额 ,
		...
		SUM(CASE WHEN HsTjDate BETWEEN @monthstartday AND @monthendday  THEN RoomTotal ELSE 0 END)/10000  AS 本月认购金额 ,
		...
		SUM(CASE WHEN HsTjDate BETWEEN @yearstartday AND @yearendday  THEN RoomTotal ELSE 0 END)/10000  AS 本年认购金额 ,
		ISNULL(SUM(ISNULL(Ts, 0)), 0)       AS 累计认购套数,
		ISNULL(SUM(ISNULL(BldArea, 0)), 0)  AS 累计认购面积,
		--ISNULL(SUM(ISNULL(RoomTotal,0)),0)	AS 认购表价,
		--ISNULL(SUM(ISNULL(OCjTotal, 0)), 0) AS 认购协议金额,
		ISNULL(SUM(ISNULL(RmbAmount, 0)), 0)/10000  AS 累计认购金额/*认购未签约购按认购协议金额，已签约的按成交金额*/
	FROM dbo.data_wide_s_SaleHsData A  WITH (NOLOCK) 
	LEFT JOIN (SELECT TradeGUID,max(TopProductTypeName) as TopProductTypeName ,
		max(qudao) as qudao, max(CDiscountRemark) as CDiscountRemark FROM data_wide_s_Trade group by TradeGUID) T ON T.TradeGUID=A.TradeGUID
...
HK AS (
	SELECT
		...
		SUM(CASE WHEN Getin.ItemType='贷款类房款' AND Getin.IsFk = 1 THEN Getin.RmbAmount ELSE 0 END)/10000  AS 累计贷款类回款
			...
			*/
			FROM
		(select Getin.ProjGUID,Getin.ItemType,Getin.IsFk,Getin.RmbAmount,(case when getin.SkDate>t.CQsDate then getin.SkDate else t.CQsDate end) AS SkDate
...
SELECT 
	...
	QY.本日签约金额,
	QY.本日老带新签约套数,
	case when isnull(QY.本日签约套数,0)=0 then 0.00 else
	cast(QY.本日老带新签约套数 as decimal(24,0))/cast(QY.本日签约套数 as decimal(24,0))  end AS 本日老带新签约占比,
	case when isnull(QY.本日签约套数,0)=0 then 0.00 else
	cast(QY.本日渠道签约套数 as decimal(24,0))/cast(QY.本日签约套数 as decimal(24,0))  end AS 本日渠道签约占比,
	case when isnull(QY.本日签约套数,0)=0 then 0.00 else
	cast(QY.本日其他途径签约套数 as decimal(24,0))/cast(QY.本日签约套数 as decimal(24,0))  end AS 本日其他渠道签约占比,
	QY.本日渠道签约套数,
	QY.本日其他途径签约套数,
	QY.本日车位签约套数,
	QY.本日车位签约面积,
	...
	QY.累计渠道签约套数,
	QY.累计其他途径签约套数,
	case when isnull(QY.累计签约套数,0)=0 then 0.00 else
	cast(QY.累计老带新签约套数 as decimal(24,0))/cast(QY.累计签约套数 as decimal(24,0))  end AS 累计老带新签约占比,
	case when isnull(QY.累计签约套数,0)=0 then 0.00 else
	cast(QY.累计渠道签约套数 as decimal(24,0))/cast(QY.累计签约套数 as decimal(24,0))  end AS 累计渠道签约占比,
	case when isnull(QY.累计签约套数,0)=0 then 0.00 else
	cast(QY.累计其他途径签约套数 as decimal(24,0))/cast(QY.累计签约套数 as decimal(24,0))  end AS 累计其他途径签约占比,
	QY.累计车位签约套数,
	QY.累计车位签约面积,
	QY.累计车位签约金额,
	QY.累计住宅签约套数,
...
SELECT 
	...
	SUM(QY.本日签约金额),
	SUM(QY.本日老带新签约套数),
	case when sum(isnull(QY.本日签约套数,0))=0 then 0.00 else
	sum(cast(QY.本日老带新签约套数 as decimal(24,0)))/sum(cast(QY.本日签约套数 as decimal(24,0)))  end AS 本日老带新签约占比,
	case when sum(isnull(QY.本日签约套数,0))=0 then 0.00 else
	sum(cast(QY.本日渠道签约套数 as decimal(24,0)))/sum(cast(QY.本日签约套数 as decimal(24,0)))  end AS 本日渠道签约占比,
	case when sum(isnull(QY.本日签约套数,0))=0 then 0.00 else
	sum(cast(QY.本日其他途径签约套数 as decimal(24,0)))/sum(cast(QY.本日签约套数 as decimal(24,0)))  end AS 本日其他途径签约占比,
	SUM(QY.本日渠道签约套数),
	SUM(QY.本日其他途径签约套数),
	SUM(QY.本日车位签约套数),
	SUM(QY.本日车位签约面积),
	...
	SUM(QY.累计渠道签约套数),
	SUM(QY.累计其他途径签约套数),
	case when sum(isnull(QY.累计签约套数,0))=0 then 0.00 else
	sum(cast(QY.累计老带新签约套数 as decimal(24,0)))/sum(cast(QY.累计签约套数 as decimal(24,0)))  end AS 累计老带新签约占比,
	case when sum(isnull(QY.累计签约套数,0))=0 then 0.00 else
	sum(cast(QY.累计渠道签约套数 as decimal(24,0)))/sum(cast(QY.累计签约套数 as decimal(24,0)))  end AS 累计渠道签约占比,
	case when sum(isnull(QY.累计签约套数,0))=0 then 0.00 else
	sum(cast(QY.累计其他途径签约套数 as decimal(24,0)))/sum(cast(QY.累计签约套数 as decimal(24,0)))  end AS 累计其他途径签约占比,
	SUM(QY.累计车位签约套数),
	SUM(QY.累计车位签约面积),
	SUM(QY.累计车位签约金额),
	SUM(QY.累计住宅签约套数),
